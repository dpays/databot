...
DPAY_GENESIS_TIMESTAMP = 1451606400
...
DPAY_MAX_ACCOUNT_CREATION_FEE = 1000000000
...
def get_account_stats(conf, silent=True):
    ...
    with open(conf["snapshot_file"], "rb") as f:
        for acc in ijson.items(f, "accounts.item"):
            ...
            total_dpay += satoshis(acc["balance"])
...
def get_proportions(account_stats, conf, silent=True):
    """
    We have a fixed amount of BEX to give out, specified by total_port_balance
    This needs to be given out subject to the following constraints:
    - The ratio of vesting : liquid BEX is the same on testnet,
    ...
    total_dpay = account_stats["total_dpay"]
    account_names = account_stats["account_names"]
    num_accounts = len(account_names)

    with open(conf["snapshot_file"], "rb") as f:
        for prefix, event, value in ijson.parse(f):
            if prefix == "dynamic_global_properties.total_vesting_fund_dpay.amount":
                total_vesting_dpay = int(value)
                break
    ...
    total_port_vesting = (avail_port_balance * total_vesting_dpay) // (total_dpay + total_vesting_dpay)
    total_port_liquid = (avail_port_balance * total_dpay) // (total_dpay + total_vesting_dpay)
    vest_conversion_factor  = (DENOM * total_port_vesting) // total_vests
    dpay_conversion_factor = (DENOM * total_port_liquid ) // total_dpay
    max_vests_per_account = int(DPAY_MAX_ACCOUNT_CREATION_FEE / vest_conversion_factor * DENOM)

    if not silent:
        print("total_vests:", total_vests)
        print("total_dpay:", total_dpay)
        print("total_vesting_dpay:", total_vesting_dpay)
        ...
        print("dpay_conversion_factor:", dpay_conversion_factor)
    ...
    return {
      ...
      "dpay_conversion_factor": dpay_conversion_factor
    }
...
def create_accounts(account_stats, conf, keydb, silent=True):
    ...
    dpay_conversion_factor = proportions["dpay_conversion_factor"]
    account_names = account_stats["account_names"]
    num_accounts = len(account_names)
    ...
    with open(conf["snapshot_file"], "rb") as f:
        for a in ijson.items(f, "accounts.item"):
            ...
            transfer_amount = (satoshis(a["balance"]) * dpay_conversion_factor) // DENOM
            name = a["name"]
            fee = max(vesting_amount, min_vesting_per_account)
            fee = min(DPAY_MAX_ACCOUNT_CREATION_FEE, fee)
...
def update_accounts(account_stats, conf, keydb, silent=True):
    ...
    with open(conf["snapshot_file"], "rb") as f:
        for a in ijson.items(f, "accounts.item"):
            ...
            new_owner_auth["key_auths"] = [["AND"+k[3:], w] for k, w in new_owner_auth["key_auths"]]
            new_active_auth["key_auths"] = [["AND"+k[3:], w] for k, w in new_active_auth["key_auths"]]
            new_posting_auth["key_auths"] = [["AND"+k[3:], w] for k, w in new_posting_auth["key_auths"]]
...
def build_actions(conf, silent=True):
    ...
    genesis_time = datetime.datetime.utcfromtimestamp(DPAY_GENESIS_TIMESTAMP)
    ...
    predicted_block_count += TRANSACTION_WITNESS_SETUP_PAD + (predicted_transaction_setup_seconds // DPAY_BLOCK_INTERVAL)

    now = datetime.datetime.utcnow()
    start_time = now - datetime.timedelta(seconds=predicted_block_count * DPAY_BLOCK_INTERVAL)
    miss_blocks = int((start_time - genesis_time).total_seconds()) // DPAY_BLOCK_INTERVAL
...
def main(argv):
    parser = argparse.ArgumentParser(prog=argv[0], description="Generate transactions for dPay testnet")
