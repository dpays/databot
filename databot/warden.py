...
def main(argv):
    ...
    parser.add_argument("-s", "--server", default="https://greatbase.dpaynodes.com", dest="server", metavar="URL", help="Specify dpayd server to watch over")
    args = parser.parse_args(argv[1:])

    backend = DPayRemoteBackend(nodes=[args.server], appbase=True, max_timeout=0.0, max_retries=0)
    dpayd = DPayInterface(backend)
    passfail = []

    config = dpayd.database_api.get_config(x=None)
    ...
    witness_schedule = dpayd.database_api.get_witness_schedule(x=None)
    witnesses = witness_schedule["current_shuffled_witnesses"]
    initminer = config["DPAY_INIT_MINER_NAME"]
    ...
    if scheduled_witnesses == config["DPAY_MAX_WITNESSES"]:
        print("[√] scheduled witnesses: %s" % config["DPAY_MAX_WITNESSES"])
        passfail.append(PREFLIGHT_GO)
    else:
