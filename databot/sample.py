...
def main(argv):
    ...
    if args.infile == "-":
        ...
        fund = snapshot["dynamic_global_properties"]["total_vesting_fund_dpay"]
        infile.seek(0)
        for prefix, event, value in ijson.parse(infile):
            if prefix == "dynamic_global_properties.total_vesting_fund_dpay.amount":
                fund["amount"] = value
            elif prefix == "dynamic_global_properties.total_vesting_fund_dpay.precision":
                fund["precision"] = value
            elif prefix == "dynamic_global_properties.total_vesting_fund_dpay.nai":
                fund["nai"] = value
            if len(fund.keys()) > 2:
                break
